...
print('Select Method...')
algo = surprise.prediction_algorithms.matrix_factorization.NMF(n_factors=15, n_epochs=30, verbose=True, biased=True, lr_bu=.00000005, lr_bi=.00000005,reg_pu=.05, reg_qi=.05)
...
temp <- df_main %>%
  ...
  select(COUNTY,parcelid,price_change_svd,freq_svd, latitude,longitude,calculatedfinishedsquarefeet, lotsizesquarefeet,bed,bath,transactiondate, topic_id,location_id, original_price) %>%
  na.omit() %>% 
...
# Modified Segmented Regression Kriging
###########################################################################
library(stats)
library(outliers)
