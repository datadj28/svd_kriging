...
# Prep for SVD - Incorporate all Zillow as Train
###########################################################################
...
# save csv and load into SVD with SGD
...
# Joining SVD Results with Canonical Matrix
###########################################################################
# ntegrate SVD
...
ml_spat <- df_main %>%
  ...
  select(COUNTY,parcelid,price_change_svd,freq_svd, latitude,longitude,calculatedfinishedsquarefeet,bed,bath,transactiondate, original_price) %>% 
  na.omit() %>% 
...
test <- ml_spat %>% filter(outlier==TRUE)
mae(test$original_price, test$freq_svd)

rm(df_main)
gc()

fwrite(ml_spat, 'data/ml_spat.csv', sep=",")
...
for (i in county_list){
  ...
  lzn.vgm <- variogram(original_price ~ log(freq_svd)+freq_svd_e, data) # calculates sample variogram values 
  
  # automatically chooses the best model
  lzn.fit = fit.variogram(lzn.vgm, vgm(c("Exp", "Gau", "Sph", "Mat")), fit.kappa = TRUE)
  
  coordinates(grid) <- ~ longitude + latitude
  lzn.kriged <-krige(original_price ~ log(freq_svd), data, grid, model = lzn.fit,maxdist=.75)
...
mae(data_new$original_price, data_new$freq_kriging)
mae(data_new$original_price, data_new$freq_svd)
...
# Kriging with Clustering for County ID 3101
###########################################################################
library(stats)
library(outliers)
...
X <- ml_spat %>% select(latitude,longitude,freq_svd)
kmean_df <- kmeans(X, 6, iter.max = 100, nstart = 60, algorithm = c("MacQueen"))
ml_spat$group <- kmean_df$cluster
gc()


data_final = data.frame()
seed_list = c(1,2,3,4,5,6)
for (i in seed_list){
  data <- ml_spat %>% filter(group==6) %>% filter(price_change_svd==0)
  grid <- ml_spat %>% filter(group==6)%>% filter(price_change_svd==1)
  
  coordinates(data) <- ~ longitude + latitude
  lzn.vgm <- variogram(original_price ~ log(freq_svd)+freq_svd_e, data) # calculates sample variogram values , width=.5, cutoff=1
  
  gc()
  
  # automatically chooses the best model
  lzn.fit = fit.variogram(lzn.vgm, vgm(c("Exp", "Gau", "Sph", "Mat")), fit.kappa = TRUE) #, "Exp", "Gau", "Sph", "Mat"
  lzn.fit
  coordinates(grid) <- ~ longitude + latitude
  lzn.kriged <-krige(original_price ~ log(freq_svd), data, grid, model = lzn.fit, maxdist=.75) # maxdist=.75
...
# Determine new MAE for clustered approach of county id 3101
mae(data_new$original_price, data_new$freq_kriging)
# Determine MAE of baseline (SVD)
mae(data_new$original_price, data_new$freq_svd)
###########################################################################
